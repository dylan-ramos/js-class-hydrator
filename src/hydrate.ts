...
import { ConfigurableObject, PropsType, UnknownType } from './types/global'
 ...
 */
const isObjectType = (data: UnknownType): data is PropsType =>
  typeof data === 'object'
...
const hasConfigurableProperty = (
  ...
  return (
    typeof propertyDescriptor === 'object' && propertyDescriptor.configurable
  )
}
 ...
 */
const hasPropertyInDataObject = (data: PropsType, key: string): boolean =>
 ...
 */
const assignAndDeleteProperty = (
  source: PropsType,
  key: string,
  target: ConfigurableObject,
  property: string,
): void => {
  if (
    !hasPropertyInDataObject(source, key) ||
...
const hydrateObject = (
  data: PropsType,
  targetObject: ConfigurableObject,
): void => {
  let target = targetObject

  do {
    Object.entries(data).forEach(([key]): void => {
      const property: string = toCamelCase(key)
    ...
    if (hasConfigurableProperty(target, 'props')) {
      target.props = {
        ...(target.props as ConfigurableObject),
        ...data,
      } as ConfigurableObject
    }

    target = Object.getPrototypeOf(target) as ConfigurableObject
  } while (target)
}
 ...
 * @returns {void}
 */
const hydrate = function (data: PropsType): void {
  if (!isObjectType(data)) {
    throw new Error('Constructor parameter need to be object type to works')
  }

  const targetObject = this as ConfigurableObject
